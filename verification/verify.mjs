...
const reportKeys = {
  "reports/fallback_decisions.csv": ["job_id"],
  "reports/placeholder_mismatches.csv": ["template_id", "locale", "field"],
  "reports/render_samples.csv": ["job_id"],
  "reports/template_locale_matrix.csv": ["template_id", "requested_locale"]
};
...
async function prepare(label, mutate) {
  ...
  await fsp.writeFile(path.join(inputRoot, "src", "compile_push_templates.mjs"), reference.get("src/compile_push_templates.mjs"));
...
function mutateQueueCoupon(text) {
  const rows = parseCsv(text);
  const target = rows.find((row) => row.job_id === "J006");
  assert(target, "队列缺少J006");
  const variables = JSON.parse(target.variables_json);
  variables.coupon_code = "EXTRA20";
  target.variables_json = JSON.stringify(variables);
  const headers = ["job_id", "template_id", "requested_locale", "user_id", "variables_json", "consent_state", "campaign_state"];
...
for (const label of ["Q10147 第一次 空目录", "Q10147 第二次 中文 空格目录"]) {
  ...
  const before = treeDigest(prepared.inputRoot, new Set(["output", "src"]));
  const result = await runNpm(["run", "compile:push"], prepared.inputRoot);
  assert(result.code === 0, `${label}执行失败\n${result.stdout}\n${result.stderr}`);
  const after = treeDigest(prepared.inputRoot, new Set(["output", "src"]));
...
const crlf = await prepare("Q10147 CRLF 输入", async (inputRoot) => {
  const file = path.join(inputRoot, "data", "send_queue.csv");
  const text = await fsp.readFile(file, "utf8");
  await fsp.writeFile(file, text.replace(/\r?\n/gu, "\r\n"));
});
let result = await runNpm(["run", "compile:push"], crlf.inputRoot);
...
const mutation = await prepare("Q10147 补齐变量", async (inputRoot) => {
  const file = path.join(inputRoot, "data", "send_queue.csv");
  await fsp.writeFile(file, mutateQueueCoupon(await fsp.readFile(file, "utf8")));
});
result = await runNpm(["run", "compile:push"], mutation.inputRoot);
assert(result.code === 0, `补齐变量执行失败\n${result.stdout}\n${result.stderr}`);
const mutatedDecisions = normalizedRows("reports/fallback_decisions.csv", fs.readFileSync(path.join(mutation.outputRoot, "reports", "fallback_decisions.csv"), "utf8"));
const mutatedRenders = normalizedRows("reports/render_samples.csv", fs.readFileSync(path.join(mutation.outputRoot, "reports", "render_samples.csv"), "utf8"));
const changedDecision = mutatedDecisions.find((row) => row.job_id === "J006");
const changedRender = mutatedRenders.find((row) => row.job_id === "J006");
assert(changedDecision?.decision === "ready" && changedDecision?.block_reason === "", "补齐变量没有放行J006");
assert(changedRender?.locale === "zh-Hant-HK" && changedRender?.body.includes("EXTRA20"), "补齐变量没有渲染J006");
const baselineDecisions = normalizedRows("reports/fallback_decisions.csv", mutation.reference.get("reports/fallback_decisions.csv").toString("utf8"));
assert(JSON.stringify(mutatedDecisions.filter((row) => row.job_id !== "J006")) === JSON.stringify(baselineDecisions.filter((row) => row.job_id !== "J006")), "补齐变量影响了无关任务");

const negative = await prepare("Q10147 无效输入", async (inputRoot) => {
  await fsp.rm(path.join(inputRoot, "templates", "push_templates.json"));
});
result = await runNpm(["run", "compile:push"], negative.inputRoot);
...
const evidence = {
  schema_version: 1,
  task_asset_id: "node_push_locale_disposition",
  result: "PASS",
  ...
  crlf_input: { file: "data/send_queue.csv", exit_code: 0, semantic_digest: crlfDigest, reference_match: true },
  positive_mutation: {
    changed_input: "J006补入coupon_code=EXTRA20",
    exit_code: 0,
    affected_job: "J006",
    decision: changedDecision.decision,
    resolved_locale: changedDecision.resolved_locale,
    rendered: Boolean(changedRender),
    unrelated_jobs_unchanged: true
  },
  invalid_input: {
    removed_input: "templates/push_templates.json",
